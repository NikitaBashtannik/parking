...
public class TimeAndPriceController {
    ...
    @GetMapping("/{id}")
    public TimeAndPriceDto read(@PathVariable("id") Long id) throws Exception {
        log.info("Get contract by id [{}]", id);
        return timeAndPriceService.read(id);
    }

    @PostMapping("/")
    public TimeAndPriceDto create(@Valid @RequestBody TimeAndPriceDto timeAndPriceDto) {
        log.info("Add new contract with request [{}]", timeAndPriceDto);
        return timeAndPriceService.create(timeAndPriceDto);
    }

    @PutMapping("/{id}")
    public TimeAndPriceDto update(@Valid @RequestBody TimeAndPriceDto timeAndPriceDto, @PathVariable("id") Long id) throws Exception {
